...
v2 - Fixed NaN issue with gradual heating and smaller initial steps.
"""
import os
import sys
import time
...
# Use a simple Langevin integrator for minimization
...
print("  Running minimization (steepest descent + L-BFGS)...")
t0 = time.time()
# First do steepest descent
...
# Phase 2b: Equilibration at 300K
...
# Check energy and density
state = simulation_npt.context.getState(getEnergy=True)
print(f"  Final potential energy: {state.getPotentialEnergy().value_in_unit(omm_unit.kilojoules_per_mole):.1f} kJ/mol")
state = simulation_npt.context.getState(getParameter=True)
