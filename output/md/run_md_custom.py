...
Handles GAFF-parameterized ligands that don't have OpenMM templates.
"""
import os
import sys
import time
...
for res in top.residues:
    if res.name == 'LIG':
        lig_res = res
        print(f"  Found LIG residue: idx={res.idx}, atoms={len(res.atoms)}")
        break
...
# Apply restraints to protein CA and ligand heavy atoms
k_restraint = 10.0 * omm_unit.kilocalories_per_mole / omm_unit.angstroms**2
positions = top.positions

for atom in top.atoms:
    if atom.residue.name == 'LIG':
        if atom.element_name != 'H' and atom.element_name != 'hydrogen':
...
# Remove position restraints for equilibration
...
# Re-create simulation with updated system (no restraints)
integrator_nvt = mm.LangevinMiddleIntegrator(
    TEMPERATURE_K * omm_unit.kelvin,
    1.0 / omm_unit.picosecond,
    TIMESTEP_FS * omm_unit.femtoseconds,
)
...
simulation_nvt = app.Simulation(openmm_top, system, integrator_nvt)
simulation_nvt.context.setPositions(positions)
simulation_nvt.context.setVelocitiesToTemperature(TEMPERATURE_K * omm_unit.kelvin)
...
simulation_nvt.reporters.append(app.StateDataReporter(
    sys.stdout, 100, step=True, time=True, potentialEnergy=True, 
    temperature=True, progress=True, remainingTime=True,
    speed=True, totalSteps=nvt_steps, separator='\t'))

t0 = time.time()
simulation_nvt.step(nvt_steps)
t1 = time.time()
print(f"  NVT complete in {t1-t0:.1f}s")
...
simulation_npt.reporters.append(app.StateDataReporter(
    ...
    speed=True, totalSteps=npt_steps, separator='\t'))
...
simulation_prod.context.setPositions(positions)
...
try:
    ...
    ca_rmsd_mean = np.mean(ca_rmsd)
    ca_rmsd_std = np.std(ca_rmsd)
    ca_rmsd_final = ca_rmsd[-1]
    ...
    if len(lig_indices) > 0:
        ...
        lig_rmsd_mean = np.mean(lig_rmsd)
        lig_rmsd_std = np.std(lig_rmsd)
        lig_rmsd_final = lig_rmsd[-1]
    ...
    # Get top 10 most flexible residues
    ...
    if len(lig_indices) > 0:
        # Find protein-ligand hydrogen bonds
        hbonds = md.baker_hubbard(traj, freq=0.0, exclude_water=True)
        # Filter for protein-ligand pairs
        pl_hbonds = []
        for hbond in hbonds:
        ...
        if len(pl_hbonds) > 0:
            for hbond in pl_hbonds:
                ...
                h_atom = traj.topology.atom(h_idx)
        ...
        for hbond in pl_hbonds:
            ...
            for frame in range(traj.n_frames):
                ...
                if d < 0.35:  # 3.5 Angstrom
                    count += 1
            occupancy = count / traj.n_frames * 100
            hbond_occupancy.append(occupancy)
            d_atom = traj.topology.atom(d_idx)
            a_atom = traj.topology.atom(a_idx)
            print(f"    {d_atom.residue.name}{d_atom.residue.resSeq}:{d_atom.name} - "
                  f"{a_atom.residue.name}{a_atom.residue.resSeq}:{a_atom.name}: "
    ...
    results = {
        'ca_rmsd_mean': float(ca_rmsd_mean),
        'ca_rmsd_std': float(ca_rmsd_std),
        'ca_rmsd_final': float(ca_rmsd_final),
        'lig_rmsd_mean': float(np.mean(lig_rmsd)) if lig_rmsd is not None else None,
        'lig_rmsd_std': float(np.std(lig_rmsd)) if lig_rmsd is not None else None,
        'lig_rmsd_final': float(lig_rmsd[-1]) if lig_rmsd is not None else None,
        'rg_mean': float(rg_mean),
        'rg_std': float(rg_std),
        'n_frames': traj.n_frames,
        'prod_time_seconds': prod_time,
        'top10_flexible': [(ca_resnames[i], float(ca_rmsf[i])) for i in top10_idx],
    }
