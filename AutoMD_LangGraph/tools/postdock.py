...
def _safe_mol_from_mol2(path: str, removeHs: bool = False):
    """Safe MOL2 load: suppress C++ stderr via fd redirect, use sanitize=False for GAFF atom types."""
    ...
    try:
        try:
            mol = Chem.MolFromMol2File(path, sanitize=False, removeHs=removeHs)
            if mol is not None:
...
def analyze_interactions(
    ...
    if interactions is not None:
        tool_debug("[analyze_interactions] RDKit distance-based (last resort)")
...
def _rdkit_distance_interactions(
    ...
    try:
        ...
        if ligand is None:
            ligand = _safe_mol_from_mol2(ligand_sdf, removeHs=False)
        if ligand is None:
            return None
...
def cluster_docking_poses(
    ...
    try:
        ...
        return failed(errors=["RDKit not available"], env_packages=["rdkit"])
