...
from tools.shared import PROJECT_ROOT, success, degraded, failed, ToolResult, tool_debug
...
def run_md_simulation(
    ...
    Two modes:
    - Complex mode: provide complex_prmtop + complex_inpcrd (pre-built system).
    - Separate mode: provide protein_prmtop + protein_inpcrd + ligand_prmtop + ligand_inpcrd.

    Args:
        complex_prmtop: Path to pre-built complex AMBER topology.
        complex_inpcrd: Path to pre-built complex AMBER coordinates.
        protein_prmtop: Path to protein AMBER topology (separate mode).
        protein_inpcrd: Path to protein AMBER coordinates (separate mode).
        ligand_prmtop: Path to ligand AMBER topology (separate mode).
        ligand_inpcrd: Path to ligand AMBER coordinates (separate mode).
    ...
    try:
        ...
        return failed(errors=["OpenMM not installed"], env_packages=["openmm"])
...
def _build_system(
    ...
    Complex mode: if complex_prmtop/complex_inpcrd are provided, load the
    pre-built complex directly (single topology). Otherwise combine separate
    protein and ligand topologies.
    """
    try:
        from openmm import unit, app
    except ImportError:
    ...
    try:
        if complex_prmtop:
            tool_debug("[_build_system] Complex mode (pre-built solvated complex)")
            prmtop = app.AmberPrmtopFile(complex_prmtop)
            inpcrd = app.AmberInpcrdFile(complex_inpcrd)
            ...
            with open(topology_path, "w") as f:
                app.PDBFile.writeFile(prmtop.topology, inpcrd.positions, f)
        else:
            tool_debug("[_build_system] Separate mode (protein + ligand)")
        ...
        topo = prmtop.topology if complex_prmtop else modeller.topology

        return success({
            "system_xml": system_xml,
            "topology_path": topology_path,
            "n_atoms": topo.getNumAtoms(),
            "n_residues": topo.getNumResidues(),
        })
...
def _minimize_energy(
    ...
    ligand_restraint_kcal: float = 50.0,
) -> ToolResult:
    """Two-stage energy minimization: ligand-restrained then unrestrained.

    Stage 1 (restrained): harmonic restraints on ligand heavy atoms prevent
    the ligand from flying out of the binding pocket while resolving steric
    clashes from docking.

    Stage 2 (unrestrained): full system relaxation to the nearest local minimum.
    """
    try:
        from openmm import unit, app
        from openmm import XmlSerializer, VerletIntegrator, CustomExternalForce
    except ImportError:
        return failed(errors=["OpenMM not installed"], env_packages=["openmm"])
    ...
    try:
        ...
        # ---- Stage 1: Ligand-restrained minimization ----
        ligand_indices = [
            atom.index for atom in pdb.topology.atoms()
            if atom.residue.name == "LIG"
        ]

        if ligand_indices and ligand_restraint_kcal > 0:
            restraint_force = CustomExternalForce(
                "0.5 * k * ((x - x0)^2 + (y - y0)^2 + (z - z0)^2)"
            )
            restraint_force.addGlobalParameter(
                "k",
                ligand_restraint_kcal * unit.kilocalories_per_mole / unit.angstroms**2,
            )
        ...
        # Quick check: ligand displacement after minimization
        if ligand_indices:
            displacements = []
            for idx in ligand_indices:
                ip = pdb.positions[idx]
                mp = min_positions[idx]
                dx = mp[0].value_in_unit(unit.angstrom) - ip[0].value_in_unit(unit.angstrom)
                dy = mp[1].value_in_unit(unit.angstrom) - ip[1].value_in_unit(unit.angstrom)
                dz = mp[2].value_in_unit(unit.angstrom) - ip[2].value_in_unit(unit.angstrom)
                displacements.append(dx*dx + dy*dy + dz*dz)
            rmsd = (sum(displacements) / len(displacements)) ** 0.5
            if rmsd > 5.0:
                warnings.append(
                    f"Ligand moved {rmsd:.1f} A during minimization "
                    f"(> 5.0 A). Check force field parameters and docking pose."
                )
            elif rmsd > 2.0:
                warnings.append(
                    f"Ligand moved {rmsd:.1f} A during minimization "
                    f"(> 2.0 A). Verify minimized structure."
                )
...
def _run_production(
    ...
    try:
        ...
        dcd_path = str(output_dir / "trajectory.dcd")
...
def _analyze_trajectory(
    ...
    try:
        ...
        protein_ca = [
            ...
            if a.residue.chain.index == 0 and a.name == "CA"
        ]
        backbone = [
            a.index for a in pdb.topology.atoms()
            if a.residue.chain.index == 0 and a.name in ("N", "CA", "C")
        ]
        ligand_heavy = [
            a.index for a in pdb.topology.atoms()
            if a.residue.chain.index != 0 and a.element is not None and a.element.mass > 1.1
        ]
...
def _add_position_restraints(
    system: Any, topology: Any, force_constant: float, reference_positions: Any
) -> None:
    ...
    for atom in topology.atoms():
        if atom.name in ("N", "CA", "C") and atom.residue.chain.index == 0:
